...
namespace agent47 {
    class Agent {
      ...
      public:
        ...
        explicit Agent(robot::Robot robot, Bridge *bridge) : robot_(std::move(robot)) {
            if (bridge) {
        ...
        inline void update(const types::Feedback &fb) {
            robot_.runtime.odom.pose = fb.pose;
            robot_.runtime.odom.twist = fb.twist;

            odom = robot_.runtime.odom;
        }
        ...
        inline dp::Result<dp::Stamp<types::Command>> tick(dp::f64 dt_s) {
            ...
            if (!have_fb) {
                ...
                fb.value.pose = robot_.runtime.odom.pose;
                fb.value.twist = robot_.runtime.odom.twist;
            }
            auto result = tick(fb);
        ...
        inline dp::Result<dp::Stamp<types::Command>> tick(const dp::Stamp<types::Feedback> &fb) {
            ...
            types::Command out;
            if (twist.has_value()) {
                auto s = static_cast<dp::f64>(robot_.runtime.speed_scale);
            ...
            st.allow_move = robot_.runtime.allow_move;
            st.allow_reverse = robot_.runtime.allow_reverse;
